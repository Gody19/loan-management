<!-- Sidebar Component -->
<aside class="hidden md:flex flex-col h-screen fixed left-0 top-0 p-md space-y-md bg-surface-container-low border-r border-outline-variant w-64 z-40">
    ...
    <div class="mt-xl px-md">
        <button class="w-full bg-primary text-on-primary rounded-lg py-md px-lg font-bold flex items-center justify-center gap-sm hover:opacity-90 transition-all active:scale-95 duration-150">
    ...
    <nav class="flex-1 mt-md space-y-1">
        ...
        <a class="flex items-center gap-md {{ request()->routeIs('repayments.index') ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-high' }} rounded-lg px-md py-sm transition-all duration-100" 
    ...
    <div class="pt-lg border-t border-outline-variant space-y-1">
        ...
        <form method="POST" action="{{-- route('logout') --}}" class="w-full">
            @csrf
            <button type="submit" class="flex items-center gap-md text-on-surface-variant px-md py-sm hover:bg-surface-container-high rounded-lg transition-all w-full">
